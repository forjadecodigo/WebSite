...
// Company blue glow color (RGB): rgba(21, 67, 176, 1)
const DEFAULT_GLOW_COLOR = '21, 67, 176'
...
const ParticleCard = ({
  ...
  }, [animateParticles, clearAllParticles, disableAnimations, enableTilt, enableMagnetism, clickEffect, glowColor, alwaysAnimate])
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (spotlightRef.current?.parentNode) spotlightRef.current.parentNode.removeChild(spotlightRef.current)
    }
          ...
          if (enableStars) {
            return (
              <ParticleCard
                ...
                {...cardProps}
                disableAnimations={shouldDisableAnimations}
                particleCount={isMobile ? Math.max(10, particleCount) : Math.max(18, particleCount)}
                glowColor={glowColor}
              ...
              >
                <div className="card__content">
                  ...
                  {card.iconClass && (
                    <div className={`card__icon ${card.title === 'Software Empresarial' ? 'card__icon--lower' : ''}`}>
